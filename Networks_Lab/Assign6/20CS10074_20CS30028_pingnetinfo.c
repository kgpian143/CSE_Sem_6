                ...
                char *ping_dom, char *ping_ip, char *rev_host)
{
    ...
    clock_gettime(CLOCK_MONOTONIC, &tfs); // CLOCK_MONOTONIC is used to measure time
    ...
    while(pingloop)
    {
        ...
        else
        {
            ...
            if(flag)
            {
                ...
                {
                    printf("Error..Packet received with type %d code %d\n", pckt.hdr.type, pckt.hdr.code);
                }
                else
                {
    ...
    double bandwidth = (msg_received_count * PING_PKT_S * 8) / total_msec;
    printf("\n===%s ping statistics===\n", ping_ip);
